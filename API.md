...
|--------|-----------------|---------------------|
| Паспорт через LLM | `POST /v1/convert` | поле `llm_context`, внутри — `part_type` и `designation` |
...
### Интеграция без оригинального имени файла

Многие backend-клиенты отправляют DXF как `drawing.dxf` — **это нормально**.

Конвертер **не зависит** от имени загружаемого файла:

| Поле | Откуда берётся |
|------|----------------|
| `designation` | штамп DXF (блоки `U*`, основная надпись) → тексты чертежа → имя файла (fallback) |
| `part_type` | штамп DXF → тексты чертежа → имя файла (fallback) |
| `product_name` | то же, что `part_type` (для совместимости с промптами) |

Поддерживаемые форматы обозначения: `42-2`, `42-2-01`, `07-54-105`, `22/02`, `22/02-01`.

Имена вроде `drawing.dxf`, `input.dxf`, `upload.dxf` **игнорируются** при fallback — чтобы не подставлять мусор.

Минимальный клиент:

```bash
curl -s -X POST "http://localhost:8000/v1/convert" \
  -F "file=@any/path/file.dxf;filename=drawing.dxf" \
  -F "render_png=false" \
  | jq '{designation, part_type, status: .validation_gate.status, llm: (.llm_context | length)}'
```
...
| 3 | Тексты чертежа | `Ролик` в надписи на поле |
| 4 | Имя файла | только fallback, если штамп не распознан |

## Источники `designation` (обозначение)

| Приоритет | Источник | Пример |
|-----------|----------|--------|
| 1 | **Штамп DXF** (блоки `U*`) | `42-2`, `07-54-105`, `22/02` |
| 2 | Тексты чертежа (с оценкой кандидатов) | таблица исполнений |
| 3 | Имя файла | только если не `drawing.dxf` / `input.dxf` |

Номера ГОСТ (`5950-2000`, `4543-71`) и допуски (`T 0,05`) **не** используются как обозначение.
