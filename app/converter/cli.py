#!/usr/bin/env python3
"""CLI entrypoint: python -m app.converter.cli --dxf file.dxf"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--out-dir", default="output", help="Каталог результатов"
    )
    parser.add_argument(
        "--name", default=None, help="Базовое имя файлов (без расширения)"
    )
    parser.add_argument(
        "--skip-png", action="store_true", help="Не рендерить PNG"
    )
    parser.add_argument(
        "--png-dpi", type=int, default=300, help="DPI для PNG (default: 300)"
    )
    parser.add_argument(
        "--dxf-text-policy",
